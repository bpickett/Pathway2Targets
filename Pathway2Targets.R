...
#setwd("~/Library/CloudStorage/Box-Box/Work/Collaborations/Tristan/CRC/")
#setwd("~/Downloads/")
infile <- args[1]
#infile <- "CRC_edgeR.txt_entrez.tsv_2022-10-08_08-23-30_SPIA_Results.csv"
#infile <- "enrichr_results1.csv" #enrichr data
#infile <- "edgeR_dge.rds-TreatmentDengueSev-TreatmentHealthy_2024-11-27_21-15-24.44598_SPIA_Results.csv-TreatmentDengueSev-TreatmentDengue_2024-11-27_23-06-04.364637_SPIA_Results.csv"
outfile <- paste0(infile,"-Treatments.tsv")
outfile1 <- paste0(infile,"-RankedTargets.tsv")
setwd("/nobackup/archive/grp/grp_scRNA/spia")
#setwd("~/")
...
sig_paths <- as.vector(in_data$GENE.SET)
sig_dbs <- as.vector(in_data$SourceDB)
ensembl_gene_id <- as.vector(NULL)
...
for(i in 1:length(sig_dbs)){
  ...
  if(sig_dbs[i] == "KEGG"){
    if(length(keggHuman.gs[[sig_paths[i]]])==0){
      print(paste0("Skipping...Pathway ",i," of ", length(sig_dbs)," : ",sig_paths[i]," -- No Protein Data"))
      next()
    }
    db_entrezID <- as.vector(keggHuman.gs[[sig_paths[i]]])
    ...
    for(w in 1:length(db_entrezID)){
      tempID <- xx[[db_entrezID[w]]]
      #print(tempID)
      ensembl_gene_id <- append(ensembl_gene_id,tempID)
      #write new row in genes df
    }
  }else if(sig_dbs[i] == "Reactome"){
    #print("in Reactome loop")
    if(length(reactomeHuman.gs[[sig_paths[i]]])==0){
      print(paste0("Skipping...Pathway ",i," of ", length(sig_dbs)," : ",sig_paths[i]," -- No Protein Data"))
      next()
    }
    db_entrezID <- as.vector(reactomeHuman.gs[[sig_paths[i]]])
...
#    db_entrezID <- db_entrezID[!duplicated(db_entrezID)]
  ...
  genes <- db_entrezID
  ensembl_vector <- ensembl_gene_id
  ...
  for(k in 1:length(ensembl_vector)){
    ...
    target_data = jsonlite::fromJSON(char)
    #target_data <- content(r)$data
    
    #if(length(target_data[["data"]][["drug"]] > 0)){
    if(length(unlist(target_data[["data"]][["target"]][["knownDrugs"]][["rows"]])) == 0){
      ...
      for(y in 1:length(target_data[["data"]][["target"]][["knownDrugs"]][["rows"]][["drug"]])){
        #y <- 1
        tractability <- 0
        unique_chembl <- as.vector(NULL)
        inter_col_df <- unique(target_data[["data"]][["target"]][["knownDrugs"]][["rows"]][["drug"]])
        inter_col <- as.character(as.vector(unlist(inter_col_df[y,])))
        #inter_col <- as.character(inter_col)
        #inter_col <- unique(inter_col)
        #sum tractability at "approved", "advanced clinical trials", or "phase 1 trials" for SM, Ab, PR, and OC
        for(z in 1:length(trac_vector)){
          #z <- 1
          if(length(target_data[["data"]][["target"]][["tractability"]])==0){
            sm_approved <- "Unknown"
            ...
            oc_phase1 <- "Unknown"
          }else if(length(target_data[["data"]][["target"]][["tractability"]])>0){
            sm_approved <- target_data[["data"]][["target"]][["tractability"]][["value"]][[1]]
            sm_advancedClinical <- target_data[["data"]][["target"]][["tractability"]][["value"]][[2]]
            sm_phase1 <- target_data[["data"]][["target"]][["tractability"]][["value"]][[3]]
            ab_approved <- target_data[["data"]][["target"]][["tractability"]][["value"]][[9]]
            ab_advancedClinical <- target_data[["data"]][["target"]][["tractability"]][["value"]][[10]]
            ab_phase1 <- target_data[["data"]][["target"]][["tractability"]][["value"]][[11]]
            pr_approved <- target_data[["data"]][["target"]][["tractability"]][["value"]][[18]]
            pr_advancedClinical <- target_data[["data"]][["target"]][["tractability"]][["value"]][[19]]
            pr_phase1 <- target_data[["data"]][["target"]][["tractability"]][["value"]][[20]]
            oc_approved <- target_data[["data"]][["target"]][["tractability"]][["value"]][[26]]
            oc_advancedClinical <- target_data[["data"]][["target"]][["tractability"]][["value"]][[27]]
            oc_phase1 <- target_data[["data"]][["target"]][["tractability"]][["value"]][[28]]
            if(target_data[["data"]][["target"]][["tractability"]][["value"]][[z]]=="TRUE"){
              tractability <- tractability+1
            }
          }
        }
        if(length(target_data[["data"]][["target"]][["subcellularLocations"]])==0){
          subCellLoc <- "No data"
        }else if(length(target_data[["data"]][["target"]][["subcellularLocations"]])>0){
          subCellLoc <- as.vector(unlist(target_data[["data"]][["target"]][["subcellularLocations"]][[1]][[1]]))
        }
        ...
        if(length(unique_chembl) == 0){
          ...
          # }
          temp_row <- as.vector(c(target_data[["data"]][["target"]][["id"]],target_data[["data"]][["target"]][["approvedSymbol"]],target_data[["data"]][["target"]][["approvedName"]],target_data[["data"]][["target"]][["associatedDiseases"]][["count"]],tractability,sm_approved,sm_advancedClinical,sm_phase1,ab_approved,ab_advancedClinical,ab_phase1,pr_approved,pr_advancedClinical,pr_phase1,oc_approved,oc_advancedClinical,oc_phase1,subCellLoc,length(target_data[["data"]][["target"]][["safetyLiabilities"]]),target_data[["data"]][["target"]][["knownDrugs"]][["uniqueDrugs"]],inter_col,sig_dbs[i], sig_paths[i]))
          ...
          temp_row <- as.vector(c(target_data[["data"]][["target"]][["id"]],target_data[["data"]][["target"]][["approvedSymbol"]],target_data[["data"]][["target"]][["approvedName"]],target_data[["data"]][["target"]][["associatedDiseases"]][["count"]],tractability,target_data[["data"]][["target"]][["tractability"]][[1]][["value"]],target_data[["data"]][["target"]][["tractability"]][[2]][["value"]],target_data[["data"]][["target"]][["tractability"]][[3]][["value"]],target_data[["data"]][["target"]][["tractability"]][[9]][["value"]],target_data[["data"]][["target"]][["tractability"]][[10]][["value"]],target_data[["data"]][["target"]][["tractability"]][[11]][["value"]],target_data[["data"]][["target"]][["tractability"]][[18]][["value"]],target_data[["data"]][["target"]][["tractability"]][[19]][["value"]],target_data[["data"]][["target"]][["tractability"]][[20]][["value"]],target_data[["data"]][["target"]][["tractability"]][[26]][["value"]],target_data[["data"]][["target"]][["tractability"]][[27]][["value"]],target_data[["data"]][["target"]][["tractability"]][[28]][["value"]],subCellLoc,length(target_data[["data"]][["target"]][["safetyLiabilities"]]),target_data[["data"]][["target"]][["knownDrugs"]][["uniqueDrugs"]],inter_col,sig_dbs[i], sig_paths[i]))
          temp_row <- as.data.frame(t(temp_row))
